  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (!receiver.requests.includes(sender._id) && !receiver.friends.includes(sender._id)) {
        ...
        return res.status(400).json({ message: "Friend request had been already sent." });
      }
    }
    else {
      return res.status(400).json({ message: "You can't send friend request yourself!" });
    }
  }
  catch (error) {
    res.status(500).json({ message: error.message });
